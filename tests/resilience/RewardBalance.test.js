// tests/resilience/RewardBalance.test.js — suite V1.18.0 (valeur mise à jour V1.21.0): verrouille la
// valeur recalibrée de BALANCE.delivery.rewardDistanceMult (40) ET vérifie qu'elle se propage
// FIDÈLEMENT dans le calcul réel d'une récompense de livraison (engine/DeliveryEngine.js#createDelivery)
// — pas seulement que la constante vaut 40 dans data/Balance.js, ce qui ne prouverait rien sur son
...
await test('BALANCE.delivery.rewardDistanceMult is locked to the recalibrated value (100 -> 50 -> 55 -> 40)', async () => {
  assertEqual(BALANCE.delivery.rewardDistanceMult, 40, 'rewardDistanceMult must stay at the V1.21.0 recalibrated value — any future change here needs a deliberate, verified balancing pass, not an accidental edit');
});
...
await test('a standard-cargo delivery reward is computed exactly as distance * rewardDistanceMult * reputation factor (no vehicle/infra/network/historic/field-structure bonus on a fresh game)', async () => {
  ...
  assertEqual(d.reward, expected, `reward must be computed from the recalibrated rewardDistanceMult (40), got ${d.reward}, expected ${expected} (distance=${distance}, reputation=${game.reputation})`);
});
